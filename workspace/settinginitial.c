...
void Settinginitial()
{
	...
	Modulesetting();// 사용할 모듈 초기 세팅

	Assignment(); 	// 모듈 문제로 인한 초기 할당 및 할당해제가 필요하여 삽입
	Remove();		// 모듈 문제로 인한 초기 할당 및 할당해제가 필요하여 삽입

	Assignment();	// 핀 할당

	pwm->set_duty_cycle(pwm_pins[led0].handle,0); // 키패드의 불을 꺼진상태로 세팅
}
